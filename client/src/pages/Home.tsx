    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-emerald-500/30">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16">
            <div className="lg:w-1/2 text-left">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Button size="lg" variant="outline" className="h-14 px-8 rounded border-white/10 hover:bg-white/5 text-slate-300 hover:text-white font-medium text-lg">
                  {t('hero.demo')}
                </Button>
              </div>
            ...
            <div className="lg:w-1/2 relative">
              <div className="relative rounded bg-[#0A0A0A] border border-white/10 shadow-2xl overflow-hidden group">
                {/* Terminal Header */}
              ...
              <motion.div 
                ...
                className="absolute -right-4 top-10 bg-[#111] border border-white/10 p-4 rounded shadow-xl hidden md:block"
              >
                <div className="flex items-center gap-3 mb-1">
                  <Activity className="w-4 h-4 text-emerald-500" />
                  <span className="text-xs font-mono text-slate-400">{t('hero.rps')}</span>
                </div>
        ...
        <div className="container mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-white/5">
            {[
              ...
              <div key={i} className="py-6 px-4 text-center hover:bg-white/5 transition-colors">
      ...
      <section id="features" className="py-24 bg-[#050505]">
        <div className="container px-4 mx-auto">
          <div className="grid md:grid-cols-3 gap-8">
            <div className="p-6 rounded border border-white/10 bg-white/[0.02] hover:border-emerald-500/30 transition-colors group">
      ...
      <section id="pricing" className="py-24 border-t border-white/5 bg-[#080808]">
        <div className="container px-4 mx-auto max-w-5xl">
          ...
          <div className="grid md:grid-cols-3 gap-0 border border-white/10 rounded overflow-hidden">
            {[
              ...
              <div key={i} className={`p-8 ${plan.active ? 'bg-white/5 relative z-10' : 'bg-[#050505]'} border-r border-white/10 last:border-r-0 flex flex-col`}>
                {plan.active && <div className="absolute top-0 inset-x-0 h-1 bg-emerald-500" />}
                <h3 className="text-lg font-mono font-medium text-slate-300 mb-2">{plan.name}</h3>
                <div className="text-4xl font-bold text-white mb-8">{plan.price}<span className="text-sm font-normal text-slate-500">/mo</span></div>
                ...
                <Button variant={plan.active ? "default" : "outline"} className={`w-full ${plan.active ? 'bg-emerald-600 hover:bg-emerald-500' : 'border-white/10 hover:bg-white/5'}`}>
                  Select
                </Button>
              </div>
            ))}
      ...
      <footer className="py-8 border-t border-white/10 bg-[#020202] text-center text-xs text-slate-600 font-mono">
