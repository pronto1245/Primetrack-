...
export function AdvertiserFinance() {
  ...
  const { data: analytics, isLoading: analyticsLoading, isError: analyticsError, error: analyticsErrorData, refetch: refetchAnalytics } = useQuery<{
    ...
    publisherBreakdown: { publisherId: string; publisherName: string; publisherShortId?: string; publisherFullName?: string | null; revenue: number; payouts: number; profit: number; roiPercent: number; ftdCount: number }[];
  }>({
                            ...
                            <tbody>
                              {analytics.publisherBreakdown.map((p) => (
                                <tr key={p.publisherId} className="border-t border-border">
                                  <td className="py-1.5 text-foreground">{p.publisherShortId ? `${p.publisherShortId}${p.publisherFullName ? ` - ${p.publisherFullName}` : ''}` : '-'}</td>
                    ...
                    <div className="flex flex-col gap-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-medium text-foreground" data-testid={`text-publisher-name-${request.id}`}>{request.publisherShortId ? `${request.publisherShortId}${request.publisherFullName ? ` - ${request.publisherFullName}` : ''}` : '-'}</p>
                  ...
                  {payouts.map((payout: any) => (
                    <tr key={payout.id} className="border-b border-white/5 hover:bg-muted">
                      ...
                      <td className="py-3 px-4 text-foreground">
                        {payout.publisherShortId ? `${payout.publisherShortId}${payout.publisherFullName ? ` - ${payout.publisherFullName}` : ''}` : '-'}
                      </td>
                      <td className="py-3 px-4">
                        <Badge
                          className={
