      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <div className="overflow-x-auto -mx-2 px-2 pb-2">
          <TabsList className="inline-flex w-auto min-w-full md:grid md:w-full md:grid-cols-10 gap-1">
            ...
            <TabsTrigger value="crypto" className="flex items-center gap-2 whitespace-nowrap data-[state=active]:bg-orange-500 data-[state=active]:text-white" data-testid="tab-crypto">
              <Wallet className="h-4 w-4" />
              <span className="hidden sm:inline">Крипто</span>
            </TabsTrigger>
          </TabsList>
        </div>
...
function CryptoKeysTab() {
  ...
  const { data: keys = [], isLoading } = useQuery<any[]>({
    queryKey: ["/api/advertiser/crypto/keys"],
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (data: { exchange: string; apiKey: string; secretKey: string; passphrase?: string; name: string }) =>
      apiRequest("/api/advertiser/crypto/keys", "POST", data),
    onSuccess: () => {
      toast({ title: "API ключи сохранены" });
      queryClient.invalidateQueries({ queryKey: ["/api/advertiser/crypto/keys"] });
      setShowForm(false);
      setApiKey("");
      setSecretKey("");
      setPassphrase("");
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiRequest(`/api/advertiser/crypto/keys/${id}`, "DELETE"),
    onSuccess: () => {
  ...
  const exchanges = [
    ...
    { id: "okx", name: "OKX", supported: false, needsPassphrase: true },
    { id: "kraken", name: "Kraken", supported: false },
    { id: "coinbase", name: "Coinbase", supported: false, needsPassphrase: true },
    { id: "exmo", name: "EXMO", supported: false },
    { id: "mexc", name: "MEXC", supported: false },
  ];
  ...
  const handleSave = () => {
    ...
    saveMutation.mutate({ 
      ...
      name: `${selectedExchange.charAt(0).toUpperCase() + selectedExchange.slice(1)} API Key`
    });
  };
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            API ключи крипто-бирж
          </CardTitle>
          <CardDescription>
            Настройте API ключи для автоматических крипто-выплат партнёрам
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
            ...
            {exchanges.map((ex) => (
              <div
                ...
                className={`p-3 border rounded-lg text-center ${
                  status[ex.id] ? "border-green-500 bg-green-500/10" : "border-muted"
                }`}
              >
          ...
          {keys.length > 0 && (
            <div className="space-y-3">
              <h4 className="font-medium">Сохранённые ключи</h4>
              {keys.map((key: any) => (
                <div key={key.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div>
                    <div className="font-medium">{key.name}</div>
                    <div className="text-sm text-muted-foreground">
                      {key.exchange} • {key.isActive ? "Активен" : "Неактивен"}
                    </div>
                  </div>
                  <Button
                    variant="ghost"
          ...
          {!showForm ? (
            <Button onClick={() => setShowForm(true)} data-testid="button-add-crypto-key">
            ...
            <div className="space-y-4 p-4 border rounded-lg">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {exchanges.filter(e => e.supported).map((ex) => (
                    <Button
                      ...
                      data-testid={`select-exchange-${ex.id}`}
                    >
                      {ex.name}
                    </Button>
                  ))}
              ...
              {selectedExchange && (
                <>
                  ...
                  {selectedExchangeInfo?.needsPassphrase && (
                    <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Button onClick={handleSave} disabled={saveMutation.isPending} data-testid="button-save-crypto-key">
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm">
                ...
                <ol className="list-decimal list-inside space-y-1 text-muted-foreground">
                  <li>Войдите в аккаунт биржи</li>
                  <li>Перейдите в раздел API Management</li>
                  <li>Создайте новый API ключ с правами на вывод</li>
