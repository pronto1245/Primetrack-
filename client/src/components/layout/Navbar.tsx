  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-200 border-b ${
      ...
      <div className="container mx-auto px-4 flex items-center justify-between">
        ...
        {/* Brand Logo */}
        <Link href="/" className="relative flex items-center group" style={{ height: '32px' }}>
          <img src={primetrackLogo} alt={platformName} className="absolute left-0 top-1/2 -translate-y-1/2 h-16 object-contain" />
        </Link>
        ...
        <div className="hidden md:flex items-center gap-8 text-xs font-mono font-medium text-muted-foreground uppercase tracking-widest">
          ...
          <a href="#" className="hover:text-foreground transition-colors">{t('nav.apiDocs')}</a>
        </div>
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <Link href="/register/advertiser">
            <Button size="sm" className="h-8 bg-white text-black hover:bg-muted text-xs font-bold font-mono px-4 rounded-sm">
               {t('nav.start')} <ArrowRight className="w-3 h-3 ml-2" />
            </Button>
          </Link>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-background border-b border-border p-4 shadow-xl flex flex-col gap-4">
          <a href="#features" className="text-sm font-mono text-muted-foreground py-2">{t('nav.features')}</a>
          <a href="#pricing" className="text-sm font-mono text-muted-foreground py-2">{t('nav.pricing')}</a>
          <Link href="/login">
            <Button className="w-full bg-emerald-600 hover:bg-emerald-500 text-foreground font-mono">{t('nav.loginSystem')}</Button>
          </Link>
        </div>
      )}
    </nav>
